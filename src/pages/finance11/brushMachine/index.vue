    ...
    <div class="main">
      ...
      <div class="mainContent">
        ...
        <commonTable
          ...
          :isFooter="isFooter"
          @listenChild="getMult"
          printTableName="dai_brush_machine"
          :jieSuanStatus="true"
          :daiShuaStatus='true'
          @refresh="updateList"
        ></commonTable>
...
export default {
  ...
  methods: {
    ...
    newAdd() {
      const _this = this;
      _this.$axios
        .post("/project/daiBrushMachineAdd")
        .then(res => {
          if (res.data.code === 1) {
            let newUrl =
    ...
    getList(ntype) {
      this.$axios
        .post("/project/daiBrushMachineList", {
    ...
    deleteList() {
      if (this.multSelectData.length > 0) {
        this.$confirm("此操作将永久删除该审批单, 是否继续?", "提示", {
          ...
          .then(() => {
            this.$axios
              .post("/project/daiBrushMachineDel", {
    ...
    exportList() {
      ...
      if (_this.multSelectData.length > 0) {
        _this.$axios
          .post("/project/daiBrushMachineDownload", {
