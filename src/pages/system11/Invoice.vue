<template>
  <div id="Invoice">
    <div class="invoice-header-back">
      <el-button @click="blackBtn" icon="el-icon-arrow-left">返回</el-button>
    </div>
    ...
    <div class="title">
      <div class="invoice-header-wrap">
        ...
        <div class="invoice-header-item" style="width: 30%">
          <div class="invoice-header-item-name">购买时长：</div>
          <div class="invoice-header-item-value">
            {{ orderData.duration }}
          </div>
        </div>
    ...
    <div v-if="current == 0" class="invoice-submit-container">
      ...
      <el-form ref="invoiceForm" :model="invoiceForm" label-width="120px">
        ...
        >
          <el-radio-group @change="radioChange" v-model="invoiceForm.type">
        ...
        <div style="text-align: center">
          <el-button type="primary" @click="submitForm('invoiceForm')">
            提交
          </el-button>
          <el-button @click="resetForm('invoiceForm')">重置</el-button>
        </div>
      </el-form>
    </div>
    ...
    <div v-else class="invoice-result-container">
      <div class="invoice-result-wrap">
        ...
        <div class="invoice-result-value" style="color: rgb(21, 188, 131)">
          开票成功
        </div>
      </div>
      <div class="invoice-result-wrap">
      ...
      <div class="invoice-result-action" style="text-align: center">
        <button type="button" class="ant-btn ant-btn-primary" @click="loadImg">
          <span>下载电子发票</span>
        </button>
      </div>
      <div
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      _this.$refs[formName].validate(valid => {
        if (valid) {
          _this.$axios.post("/order/invoicingInfoAdd", newData).then(res => {
            _this.$message({
    ...
    getInfo() {
      this.$axios
        .post("/order/invoicingInfoDetails", {
  ...
  created() {
    ...
    this.current = Number(this.$route.query.data.b_status);
    this.invoiceForm.gl_id = this.$route.query.data.id;
    this.invoiceForm.order_id = this.$route.query.data.order_id;
    if (
      this.$route.query.data.b_status == 2 ||
      this.$route.query.data.b_status == 1
    ) {
      this.getInfo();
    }
  }
